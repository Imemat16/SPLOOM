<div align="center">
  <img src="assets/ICON0.PNG" alt="Sploom Terminal Logo" width="400">
</div>
...
* **🌐 Network Parsing:** HTTP streams include carriage returns (`\r\n`) that break text rendering on the PSP. The C parser uses a custom `strtok_r` implementation to clean these before displaying data.
...
### Backend Setup
1. Navigate to the `/middleware_server` directory and run `pip install -r requirements.txt`.
...
2. Navigate to the `/psp_client` directory on your computer.
3. Open `main.c` and update the hardcoded `ip_bytes` (or `Host` header) to point to your live hosted Python backend.
